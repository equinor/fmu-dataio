...
from docutils.parsers.rst.states import (  # type: ignore
    RSTStateMachine,
    Struct,
    state_classes,
)
...
def mangle_signature(sig: str, max_chars: int = 30) -> str:
    ...
    if opts:
        if not sig:
            sig = "[%s]" % limited_join(", ", opts, max_chars=max_chars - 4)  # noqa
        elif len(sig) < max_chars - 4 - 2 - 3:
            sig += "[, %s]" % limited_join(  # noqa
        ...
        if len(sentences) == 1:
            ...
            for i in range(len(sentences)):
                ...
                if summary.endswith(WELL_KNOWN_ABBREVIATIONS):
                    ...
                    # considered as that splitting by period
                    # does not break inline markups
                    break

    # strip literal notation mark ``::`` from tail of summary
    return literal_re.sub(".", summary)
