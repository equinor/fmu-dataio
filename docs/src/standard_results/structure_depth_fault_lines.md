...
This exports the structural depth fault lines (polygons) from within RMS.
These fault line polygons represent the intersection of an horizon surface with the
modelled fault surfaces. 

Note, it is only possible to export **one single set** of depth fault lines per
...
| Output | `share/results/polygons/structure_depth_fault_lines/surfacename.csv` |
:::

## Requirements

- RMS
- depth fault lines stored in a horizon folder within RMS
...
job in RMS. This will ensure the polygons are closed and stored correctly in the horizon folder.
:::
...
`share/results/polygons/structure_depth_fault_lines/topvolantis.csv`.

This is a tabular file on `.csv` format. It contains
