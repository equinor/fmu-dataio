...
def test_files_exported_with_metadata(mock_export_class, rmssetup_with_fmuconfig):
    ...
    assert (export_folder / "topvolantis.csv").exists()
    assert (export_folder / "toptherys.csv").exists()
    assert (export_folder / "topvolon.csv").exists()

    assert (export_folder / ".topvolantis.csv.yml").exists()
    assert (export_folder / ".toptherys.csv.yml").exists()
    assert (export_folder / ".topvolon.csv.yml").exists()
...
def test_inplace_volumes_payload_validates_against_model(
    ...
    df = pd.read_csv(out.items[0].absolute_path).to_dict(orient="records")
