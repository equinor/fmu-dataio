...
if TYPE_CHECKING:
    ...
    from fmu.dataio.export.rms.grid_extracted_depth_surfaces import (
        _ExportGridExtractedDepthSurfaces,
    )
...
def mock_export_class(
...
) -> Generator[_ExportGridExtractedDepthSurfaces]:
    ...
    with mock.patch(
        "fmu.dataio.export.rms.grid_extracted_depth_surfaces.get_horizons_in_folder",
        return_value=xtgeo_surfaces,
    ):
        yield _ExportGridExtractedDepthSurfaces(mock_project_variable, "DS_extracted")


@pytest.mark.usefixtures("inside_rms_interactive")
def test_files_exported_with_metadata(
    mock_export_class: _ExportGridExtractedDepthSurfaces, rmssetup_with_fmuconfig: Path
) -> None:
    ...
    export_folder = (
        ...
        / "../../share/results/maps/grid_extracted_depth_surface/"
    )
...
def test_standard_result_in_metadata(
    mock_export_class: _ExportGridExtractedDepthSurfaces,
) -> None:
    """Test that the standard_result is set correctly in the metadata"""
    ...
    assert (
        ...
        == StandardResultName.grid_extracted_depth_surface
    )
...
def test_public_export_function(
    ...
    out = export_grid_extracted_depth_surfaces(mock_project_variable, "DS_extracted")
