...
class SumoExplorerInterface:
    ...
    def get_realization(self, realization_id: int) -> list[DataFrame]:
        searc_context_realization = self._search_context.filter(
            realization=realization_id
        )

        data_frames: list[DataFrame] = []
        for object in searc_context_realization:
            data_frame: DataFrame = object.to_pandas().replace(np.nan, None)
