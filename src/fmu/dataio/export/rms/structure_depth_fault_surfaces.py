...
class _ExportStructureDepthFaultSurfaces(SimpleExportRMSBase):
    def __init__(
        ...
        fault_surfaces = _get_fault_surfaces_from_rms(project, structural_model_name)
...
def _get_fault_surfaces_from_rms(
    ...
    if not structural_model.fault_model:
        raise ValueError(
            f"Structural model '{structural_model_name}' does not have a fault model."
        )
    ...
    for fault_name in structural_model.fault_model.fault_names:
        fault_surface = structural_model.fault_model.get_fault_triangle_surface(
            name=fault_name,
        ...
        if fault_surface is None:
            raise ValueError(
                f"Could not retrieve a triangulated surface for the fault "
                f"'{fault_name}' in structural model '{structural_model_name}'."
            )
