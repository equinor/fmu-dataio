...
class Product(BaseModel):
    ...
    """The identifying product type for this data object."""
...
class InplaceVolumesProduct(Product):
    ...
    type: Literal[enums.ProductType.inplace_volumes] = enums.ProductType.inplace_volumes
    """The identifying product type for the 'inplace_volumes' product."""

    file_schema: FileSchema = FileSchema(
        version=inplace_volumes.VERSION,
        url=inplace_volumes.SCHEMA,
    )
...
class AnyProduct(RootModel):
    ...
    the exact type being identified by the ``product.type`` field.
    """
