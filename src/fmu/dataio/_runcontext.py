...
class RunContext:
    ...
    def _establish_exportroot(self) -> Path:
        """
        Establish the rootpath. The rootpath is the folder that together with
        the share location for a file makes up the absolute export path for an object:
        absolute_path = exportroot / objdata.share_path

        The rootpath is dependent on whether this is run in a FMU context via ERT and
        ...
        3: Running RMS interactively -> rootpath/rms/model
